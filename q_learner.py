...
class QLearner(object):
    def __init__(self, state_dim, action_dim,
        ...
        self.device = device
        # self.qnet = q_network.QMLP(state_dim, action_dim, width,depth).to(device)
        self.qnet = q_network.Resnet(state_dim, action_dim,
                                    ).to(device)
        self.optimizer = optim.Adam(self.qnet.parameters(), lr=lr)
        if load_path is not None:
            self.load(load_path)
        # self.target_qnet = q_network.QMLP(state_dim,action_dim,width,depth).to(device)
        self.target_qnet = q_network.Resnet(state_dim, action_dim,
                                ).to(device)
    ...
    def predict(self, state, action, batch=True):
        ...
        # print ('state nan', torch.isnan(state).int().sum())
        ...
        # print ('value nan', torch.isnan(value).int().sum())
...
class SarsaLearner(QLearner):
    ...
    def loss(self, transitions, pi=None, beta=None):
        # print ('sarsa loss used')
        with torch.no_grad():
            sp_value = self.target_qnet(transitions.sp, transitions.ap)
            not_done = 1 - transitions.d
            # print (transitions.r)
            target_value = transitions.r * 1000 + not_done * self.discount * sp_value

        pred_value = self.qnet(transitions.s, transitions.a)
        # print (pred_value)
        # print (target_value - pred_value)
        loss = ((target_value - pred_value) ** 2).mean()
        self.loss_list.append(loss.item())

        self.logger.update('q/loss', loss.item())
        # print('sarsa learner used')
        # print ('loss:', torch.isnan(loss).int().sum())
        return loss
