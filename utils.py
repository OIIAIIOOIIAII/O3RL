...
class TruncatedStandardNormal(Distribution):
    ...
    @staticmethod
    def _inv_big_phi(x):
        # print('inv_big:',CONST_SQRT_2 * (2 * x - 1))
        # print (torch.max(CONST_SQRT_2 * (2 * x - 1)))
        # print (torch.min(CONST_SQRT_2 * (2 * x - 1)))
        # return (CONST_SQRT_2 * (2 * x - 1)).clamp(-0.99999,0.99999).erfinv()
        # print('inv_big:',torch.isinf((2 * x-1).erfinv()).int().sum())
        # print ((2 * x - 1))
        return (2 * x - 1).clamp(-0.999999, 0.999999).erfinv()
        # return (2 * x - 1).erfinv()
    ...
    def rsample(self, sample_shape=torch.Size()):
        ...
        p = p.type_as(self.a)
        # print ('p',torch.isinf(p).int().sum())
        # print ('icdf',torch.isinf(self.icdf(p)).int().sum())
        return self.icdf(p)
...
class TruncatedNormal(TruncatedStandardNormal):
    ...
    def __init__(self, loc, scale, a=0, b=1000000, eps=1e-8, validate_args=None):
    ...
    def _to_std_rv(self, value):
        if self._validate_args:
            self._validate_sample(value)
        # print ('loc',torch.isinf(self.loc).int().sum())
