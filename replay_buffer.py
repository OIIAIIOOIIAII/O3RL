...
class Replay:
    ...
    def to_json(self):
        ...
        s_list = self.a.tolist()
        r_list = self.r.tolist()
        sp_list = self.sp.tolist()
        d_list = self.d.tolist()
        ap_list = self.ap.tolist()

        return json.dumps({'state_shape':self.state_shape,'action_shape':self.action_shape ,'s':s_list,'a':a_list,'r':r_list,'sp':sp_list,'d':d_list,'has_next_action':self.has_next_action,'max_size':self.max_size,'next_slot':self.next_slot,'ap':ap_list,'length':self.length
                           })

    # def from_json(json_str):
    #     data = json.loads(json_str)
    #     return Replay(data['state_shape'],data['action_shape'],data['s'],data['a'],data['r'],)
